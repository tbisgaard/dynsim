...
def run_ethanol_water_simulation():
    ...
    u_test = {
        ...
        'feed_enthalpy':           mixture_prop.enthalpy_liquid_mixture(system.parameters, 365, 301325, np.array([0.05,0.95-1e-4-1e-4, 1e-4, 1e-4])),
        }
    ...
    t_final = 5*60*60
    sol = solve_ivp(column_models.column_ode, [0, t_final], X_test, args=(u_test, system), method='BDF')
    ...
    system2 = system_parameters.SystemParameters(
        ["Ethanol", "Water"], 
        model_liquid="unifac1p"
        )
    T=np.zeros(150)
    y=np.zeros_like(x)
    for i in range(150):
        Ti, yi = equilibrium_calculations.solve_flash(system2.parameters, pressure=P, composition_liquid=x[i,:])
        T[i]=Ti
        y[i,:]=yi
