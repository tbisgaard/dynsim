...
def density_liquid(parameters, temperature):
    ...
    A = parameters["molar_density_constant_A"]
    B = parameters["molar_density_constant_B"]
    C = parameters["molar_density_constant_C"]
    D = parameters["molar_density_constant_D"]
...
def surface_tension(parameters, temperature):
    ...
    A = parameters["surface_tension_constant_A"]
    B = parameters["surface_tension_constant_B"]
    C = parameters["surface_tension_constant_C"]
    D = parameters["surface_tension_constant_D"]
    Tc = parameters["temperature_critical"]
    A = np.array(A, ndmin=1)[None, :]
    B = np.array(B, ndmin=1)[None, :]
    C = np.array(C, ndmin=1)[None, :]
    D = np.array(D, ndmin=1)[None, :]
...
def saturation_pressure(parameters, temperature):
    ...
    A = parameters["saturation_pressure_constant_A"]
    B = parameters["saturation_pressure_constant_B"]
    C = parameters["saturation_pressure_constant_C"]
    D = parameters["saturation_pressure_constant_D"]
    E = parameters["saturation_pressure_constant_E"]
    A = np.array(A, ndmin=1)[None, :]
    B = np.array(B, ndmin=1)[None, :]
    C = np.array(C, ndmin=1)[None, :]
    D = np.array(D, ndmin=1)[None, :]
...
def enthalpy_gas(parameters, temperature, pressure):
    h_f = parameters["enthalpy_of_formation_ideal_gas"]
    A = parameters["heat_capacity_ideal_gas_constant_A"]
    B = parameters["heat_capacity_ideal_gas_constant_B"]
    C = parameters["heat_capacity_ideal_gas_constant_C"]
    D = parameters["heat_capacity_ideal_gas_constant_D"]
    E = parameters["heat_capacity_ideal_gas_constant_E"]
    A = np.array(A, ndmin=1)[None, :]
    B = np.array(B, ndmin=1)[None, :]
    C = np.array(C, ndmin=1)[None, :]
    D = np.array(D, ndmin=1)[None, :]
...
def heat_of_vaporisation(parameters, temperature):
    A = parameters["heat_of_vaporisation_constant_A"]
    B = parameters["heat_of_vaporisation_constant_B"]
    C = parameters["heat_of_vaporisation_constant_C"]
    D = parameters["heat_of_vaporisation_constant_D"]
    Tc =  parameters["temperature_critical"]
    A = np.array(A, ndmin=1)[None, :]
    B = np.array(B, ndmin=1)[None, :]
    C = np.array(C, ndmin=1)[None, :]
    ...
    T = np.array(temperature, ndmin=1)[:, None]
    Tr = T/Tc
    Tr = np.minimum(Tr, np.ones_like(Tr))  # Avoid T>Tc
    exponent = B + C*Tr  +D*Tr**2
    dhvap = A*(1-Tr)**(exponent)
